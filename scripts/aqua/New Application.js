...
$(document)
  ...
  .on({
    change: function () {
      ...
      if (!/(?<!un)employed/i.test($val)) return ($fewaTime.hide(), $fewaEst.hide())
      else if ($fewaTime.length) {
        return setTimeout(() => {
          let $emp = $('div.formField--employmentStartDate:visible');
          if ($emp.length) $emp.parent().before($fewaTime.show()).before($fewaEst.show());
        }, 0);
      }
      ...
      $fewaEst = $('<div>').attr({
        ...
        $('<span>').attr({ class: "sumTotal__label" }).html("Suggested Date: "),
        $('<span>').attr({ class: "sumTotal__value fewaValue" }).html(start)
      );

      setTimeout(() => {
        let $emp = $('div.formField--employmentStartDate:visible');
        if ($emp.length) $emp.parent().before($fewaTime).before($fewaEst);
      }, 0);
    }
...
function fewaUpdate(e) {
  ...
  if (validKeys.indexOf(e.keyCode) < 0) return reset();
  if (e.ctrlKey) {
    if (e.keyCode == 86 || e.keyCode == 88) {
      if ($this.val().length == undefined) return reset();
      $this.val($this.val().toString().replace(/\D/g, ""));
    }
  }
  if (e.shiftKey) {
    if (a(48, 10).indexOf(e.keyCode) >= 0) return reset();
  }
  ...
  for (let a = 0; a < time.length; a++) {
    switch (time[a].val.length) {
      ...
      case 1: {
        ...
        if (time[a].val > time[a].max) time[a].$.val(time[a].max)
        else if (time[a].val < time[a].min) time[a].$.val(time[a].min);
      }
      break;
      case 0:
    }
  ...
  if (month.val >= 0 && year.val >= 0 || year.val > 0) {
    ...
    if (month.val < 9) month.val = '0' + month.val;
