...
$(document).on({
    ...
    var $fewaTime = $('#fewaTime');
      ...
      setTimeout(() => {
			...
			$fewaTime.append(
        $('<div>').append(
          $('<label>').attr({'class': "label", 'for': "fewaYear"}).html('Years:'),
          $('<input>').attr({'type': "number", 'id': "fewaYear", 'min': "0", 'max': "80", 'style': "width:2rem;height:1rem;margin-left:0.3rem;margin-right:1rem;"}),
          $('<label>').attr({'class': "label", 'for': "fewaMonth"}).html('Months:'),
          $('<input>').attr({'type': "number", 'id': "fewaMonth", 'min': "0", 'max': "11", 'style': "width:2rem;height:1rem;margin-left:0.3rem;margin-right:1rem;"}),
          $('<label>').attr({'class': "label", 'for': "fewaWeek"}).html('Weeks:'),
          $('<input>').attr({'type': "number", 'id': "fewaWeek", 'min': "0", 'max': "3", 'style': "width:2rem;height:1rem;margin-left:0.3rem;margin-right:1rem;"}),
        ),
        $('<br>'),
        $('<div>').attr({'class': "formField"}).append(
          $('<div>').attr({'class': "label"}).html('Suggested start date:'),
          $('<div>').attr({'id': "fewaTimeEst"})
        )
      );
      setTimeout(() => {
				let $emp = $('div.formField--employmentStartDate').filter(':visible');
...
.on({'keydown keyup': fewaUpdate}, '#fewaWeek')
.on({'keydown keyup': fewaUpdate}, '#fewaMonth')
.on({'keydown keyup': fewaUpdate}, '#fewaYear');
...
function fewaUpdate(e) {
  ...
  if (validKeys.indexOf(e.keyCode) < 0) {
    return $this.val($this.data('fewa-val'));
  }
  if (e.ctrlKey) {
  ...
  for (let a = 0; a < time.length; a++) {
    switch (time[a].val.length) {
      default: time[a].$.val(parseInt(time[a].val.substring(0, 2)));
      case 2:
      case 1: {
